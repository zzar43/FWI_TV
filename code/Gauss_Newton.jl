...
function compute_obj_fn(received_data, c, rho, Nx, dx, Ny, dy, Nt, dt, source, source_position, receiver_position; pml_len=30, pml_coef=50)
    c = reshape(c, Nx, Ny)
    ...
    return 0.5 * norm(data_forward-received_data,2)^2
end
...
function compute_grad(received_data, c, rho, Nx, dx, Ny, dy, Nt, dt, source, source_position, receiver_position; pml_len=30, pml_coef=50)
    ...
    return 0.5 * norm(adjoint_source,2)^2, grad
end
...
function Gauss_Newton(eval_fn, eval_CG_step, x0, iterNum, min_value, max_value; alpha=1, rho=0.9, c=1e-10, maxSearchTime=5, threshold=1e-10)
    ...
    for iter = 1:iterNum
        ...
        if alpha0 == 0
            ...
            xk = update_fn(xk, alpha0, -stepk, min_value, max_value)
        end
