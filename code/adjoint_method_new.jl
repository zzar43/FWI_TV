...
function adjoint_op(wavefield_forward, y0, c, rho, Nx, dx, Ny, dy, Nt, dt, source, source_position, receiver_position; pml_len=30, pml_coef=50)
    ...
    @inbounds @sync @distributed for ind = 1:source_num
        ...
        utt = zeros(Nx, Ny, Nt)
        @views @. utt[:,:,2:end-1] = (wavefield_forward[:,:,3:end,ind]-wavefield_forward[:,:,2:end-1,ind]+wavefield_forward[:,:,1:end-2,ind]) ./ dt^2;
...
# normal operator: (QDF[u])^* QDF[u] u_0 = u_1
# input: u, y, u_0
# output: u_1
...
function normal_op(wavefield_forward, u0, c, rho, Nx, dx, Ny, dy, Nt, dt, source, source_position, receiver_position; pml_len=30, pml_coef=50)
    ...
    @inbounds @sync @distributed for ind = 1:source_num
        ...
        data_single = acoustic_solver_no_wavefield(c, rho, Nx, dx, Ny, dy, Nt, dt, source_s, source_position_s, receiver_position; pml_len=pml_len, pml_coef=pml_coef);
        
        # adjoint of single scattering
        # -1 ?
        adj_source = -data_single[end:-1:1,:];
        v, data_backward = acoustic_solver(c, rho, Nx, dx, Ny, dy, Nt, dt, adj_source, receiver_position, receiver_position; pml_len=pml_len, pml_coef=pml_coef)
        @views @. forward_wavefield_tt = 2 ./ c^3 .* forward_wavefield_tt .* v[:,:,end:-1:1];
        grad0 = sum(forward_wavefield_tt,dims=3) * dt
        u1[:,:,ind] = grad0
